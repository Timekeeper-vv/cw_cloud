...
// 启动实时模式 - 启动数据生产者并连接WebSocket
const startRealtimeMode = async () => {
  ElMessage.info('正在启动实时数据流...')
  
  try {
    // 调用后端API启动数据源（默认使用 signal-1）
    ...
    if (!response.ok) {
      console.warn('启动数据源失败，尝试直接连接WebSocket')
    }
    ...
    // 即使启动失败也尝试连接WebSocket（可能数据源已经在运行）
    connectWebSocket()
  }
}
...
const getAnomalyTypeName = (type: string): string => {
  const names = {
    ...
    'AMPLITUDE_MEAN_ANOMALY': '均值异常',
    'AMPLITUDE_STD_ANOMALY': '波动异常',
    'EXTREME_VALUE': '极值异常'
  }
